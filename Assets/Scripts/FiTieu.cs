...
public class FiTieu : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (this.dirF == FiTieu.DirFly.Right)
		{
			this.dir = Vector2.right;
		}
		else if (this.dirF == FiTieu.DirFly.Left)
		{
	...
	private void FixedUpdate()
	{
		...
		if (hit)
		{
			if (hit.collider.tag == "Wall")
			{
				...
				hit.collider.gameObject.SendMessage("Hit", hit.point, SendMessageOptions.DontRequireReceiver);
			...
			else if (hit.collider.tag == "ThungGo")
			{
				hit.collider.gameObject.SendMessage("Hit", hit.point, SendMessageOptions.DontRequireReceiver);
				this.effectScript.ToeThungGo(hit.transform.position);
				base.gameObject.SetActive(false);
			}
			...
			else if (hit.collider.tag == "PL")
			{
				...
				if (component)
				{
					if (this.dirF == FiTieu.DirFly.Left)
					{
		...
		else if (this.dirF == FiTieu.DirFly.Right)
		{
