...
public class LinhGacSung : MonoBehaviour
{
	private void Start()
	{
	...
	public void Ban()
	{
		GameObject.FindGameObjectWithTag("Effect").GetComponent<EffectController>().FireInTheGun(this.GunHead.position, this.dir);
		this.audios.clip = this.bandan;
		this.audios.Play();
		RaycastHit2D hit = Physics2D.Raycast(this.startPoint.position, this.dir, this.attackDistance, this.layer);
		if (hit && hit.collider.tag == "Player")
		{
	...
	private void DashOver()
	{
		...
		{
			this.EnemyDead = true;
			GameObject.FindGameObjectWithTag("Effect").GetComponent<EffectController>().ToeMau(base.transform.position);
			this.iDied();
		}
	}
	...
	public AudioClip napdan;

	public AudioClip bandan;
