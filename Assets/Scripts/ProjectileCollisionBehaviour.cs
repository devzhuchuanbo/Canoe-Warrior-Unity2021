...
public class ProjectileCollisionBehaviour : MonoBehaviour
{
	...
	private void Update()
	{
		...
		Vector3 vector = (!this.effectSettings.IsHomingMove) ? this.forwardDirection : this.tTarget.position;
	...
	private void GetTargetHit()
	{
		...
		RaycastHit raycastHit;
		if (component != null && this.tTarget.GetComponent<Collider>().Raycast(ray, out raycastHit, this.effectSettings.MoveDistance))
		{
			this.hit = raycastHit;
		}
	}
