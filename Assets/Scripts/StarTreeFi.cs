...
public class StarTreeFi : MonoBehaviour
{
	private void OnEnable()
	{
		...
		if (!PlayerPrefs.HasKey("FiLevel"))
		{
			PlayerPrefs.SetInt("FiLevel", 0);
			PlayerPrefs.Save();
		}
		this.fiLevel = PlayerPrefs.GetInt("FiLevel");
		this.BrightUp();
		this.DesellectAll();
		if (this.fiLevel >= 6)
		{
	...
	public void Sellect(int i)
	{
		...
		if (i == this.fiLevel + 1)
		{
			if (this.coin >= this.coinPrice[i - 1] && this.dm >= this.dmPrice[i - 1])
			{
				...
				this.up = StarTreeFi.upko.okUpDc;
			}
			else
			{
				this.upgrageText.color = this.KoDuCoin;
				this.up = StarTreeFi.upko.thieuTien;
			}
		}
		else
		{
			this.upgrageText.color = this.Toi;
			this.up = StarTreeFi.upko.upRoi;
		}
		this.ShowInfo(i);
	}
	...
	private void ShowInfo(int i)
	{
		...
		case 1:
			...
			this.infoText.text = "Critical chance +3%";
			break;
		case 2:
			this.coinPriceText.text = this.coinPrice[i - 1].ToString();
			this.dmPriceText.text = this.dmPrice[i - 1].ToString();
		...
		case 3:
			this.coinPriceText.text = this.coinPrice[i - 1].ToString();
			this.dmPriceText.text = this.dmPrice[i - 1].ToString();
			this.infoText.text = "Critical chance +4%";
			break;
		case 4:
			this.coinPriceText.text = this.coinPrice[i - 1].ToString();
			this.dmPriceText.text = this.dmPrice[i - 1].ToString();
		...
		case 5:
			this.coinPriceText.text = this.coinPrice[i - 1].ToString();
			this.dmPriceText.text = this.dmPrice[i - 1].ToString();
			this.infoText.text = "Critical chance +5%";
			break;
		case 6:
			this.coinPriceText.text = this.coinPrice[i - 1].ToString();
			this.dmPriceText.text = this.dmPrice[i - 1].ToString();
	...
	{
		if (this.up == StarTreeFi.upko.okUpDc)
		{
			this.main.ClickBuy();
			this.coin -= this.coinPrice[this.fiLevel];
			this.dm -= this.dmPrice[this.fiLevel];
			this.fiLevel++;
			PlayerPrefs.SetInt("Coin", this.coin);
			PlayerPrefs.SetInt("DM", this.dm);
			PlayerPrefs.SetInt("FiLevel", this.fiLevel);
			PlayerPrefs.Save();
			this.BrightUp();
			if (this.fiLevel >= 6)
			{
		...
		else if (this.up == StarTreeFi.upko.thieuTien)
		{
			this.main.ClickLevelSellectFalse();
		}
		else if (this.up == StarTreeFi.upko.upRoi)
		{
			this.main.ClickLevelSellectFalse();
		}
	}
